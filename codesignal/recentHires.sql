/*
Your company has many different departments. Information about the employees working in these departments is stored in the pr_department, 
it_department, and sales_department tables, which have the following structures:
...
SELECT name as names FROM
    (
        (SELECT name, 1 AS dep FROM pr_department 
        ORDER BY date_joined DESC
        LIMIT 5)
        
        UNION ALL
        
        (SELECT name, 2 AS dep FROM it_department 
        ORDER BY date_joined DESC
        LIMIT 5)
        
        UNION ALL
        
        (SELECT name, 3 AS dep FROM sales_department 
        ORDER BY date_joined DESC
        LIMIT 5)
    ) AS result
    ORDER BY dep, name;
