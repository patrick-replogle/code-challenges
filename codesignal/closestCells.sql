...
In this game each piece on the board is said to defend its nearest neighbor. The distance between two pieces is calculated simply 
as the distance between the points (x1, y1) and (x2, y2), where (x1, y1) and (x2, y2) are the coordinates of the cells occupied by 
the first and the second piece, respectively.
...
BEGIN
    SELECT 
        id AS id1,
        (SELECT id FROM positions AS p2 
        WHERE p1.id <> p2.id 
        ORDER BY POWER(p1.x - p2.x, 2) + POWER(p1.y - p2.y, 2) ASC
        LIMIT 1
        ) AS id2
