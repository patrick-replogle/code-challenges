...
Return an array in which the first element is the largest value in the row with depth 0, the second element is the largest value in the row 
with depth 1, the third element is the largest element in the row with depth 2, etc.
...
function largestValuesInTreeRows(t) {
    ...
    while (queue.length) {
        ...
        let maxValInRow = -Infinity;
        
        for (let i = 0; i < len; i++) {
            let currNode = queue.shift();
            maxValInRow = Math.max(maxValInRow, currNode.value);
            if (currNode.left) queue.push(currNode.left);
            if (currNode.right) queue.push(currNode.right);
        }
        largestValues.push(maxValInRow);
    }
