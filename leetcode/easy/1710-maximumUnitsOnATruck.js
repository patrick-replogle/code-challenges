/*
You are assigned to put some amount of boxes onto one truck. You are given a 2D array boxTypes, where boxTypes[i] = [numberOfBoxesi, 
...
var maximumUnits = function(boxTypes, truckSize) {
    ...
    for (let type of boxTypes) {
        let [quantity, units] = type;
        
        if (truckSize >= quantity) {
            output += quantity * units;
            truckSize -= quantity;
        } else {
            while (truckSize > 0 && quantity > 0) {
                output += units;
                truckSize--;
                quantity--;
            }
